...
RUN --mount=type=cache,mode=0777,target=/home/root/app/target \
    ...
    cargo build --release --bin drift-airdrop-api && cp target/release/drift-airdrop-api ./
...
FROM base_image as drift-airdrop-api
WORKDIR /app
COPY --from=builder /home/root/app/drift-airdrop-api ./
ENTRYPOINT ["./drift-airdrop-api"]
